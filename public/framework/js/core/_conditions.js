class conditions {
            ...
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
              mutation.addedNodes.forEach(node => {
                if (node.nodeType === 1 && node.classList.contains('repeatable-item')) {
                  ...
                  // Evaluar condiciones del nuevo item después de un pequeño delay
                  setTimeout(() => {
                    if (!this.isFillingForm) {
                      logger.info('core:conditions', '[observer] Nuevo item agregado, evaluando condiciones');
  ...
  static applyVisibilitySimple(formEl, fieldPath, shouldShow) {
    logger.info('core:conditions', `[applyVisibilitySimple] fieldPath: "${fieldPath}", shouldShow: ${shouldShow}`);
  ...
  static getContext(formEl, targetFieldPath, contextType) {
    switch (contextType) {
      ...
      case 'repeatable':
        ...
        logger.info('core:conditions', `[getContext] repeatable - targetFieldPath: "${targetFieldPath}", targetField:`, targetField);
        
        if (targetField) {
          // Para repeatables anidados, el targetField DEBE SER el .form-html-wrapper que envuelve DIRECTAMENTE el .form-repeatable
          // Verificar si targetField tiene la clase form-html-wrapper Y contiene un .form-repeatable como hijo directo
          const isRepeatableWrapper = targetField.classList.contains('form-html-wrapper') && 
                                      targetField.querySelector(':scope > .form-repeatable');
          
          logger.info('core:conditions', `[getContext] isRepeatableWrapper:`, isRepeatableWrapper);
          
          if (isRepeatableWrapper) {
            // Es un repeatable anidado, buscar el item del repeatable padre
            const parentItem = targetField.closest('.repeatable-item');
            logger.info('core:conditions', `[getContext] parentItem found:`, !!parentItem);
            if (parentItem) {
              logger.info('core:conditions', `[getContext] Retornando parentItem para "${targetFieldPath}"`);
              return parentItem;
            }
          }
          
          // Campo normal dentro de repeatable
          const repeatable = targetField.closest('.repeatable-item');
          logger.info('core:conditions', `[getContext] Retornando repeatable item para "${targetFieldPath}":`, !!repeatable);
          return repeatable || formEl;
        }
        logger.warn('core:conditions', `[getContext] No se encontró targetField para "${targetFieldPath}", retornando formEl`);
        return formEl;

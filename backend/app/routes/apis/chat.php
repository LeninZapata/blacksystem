...
$router->group('/api/chat', function($router) {
  ...
  $router->get('/rebuild/{number}/{bot_id}', function($number, $bot_id) {
    log::info("chat/rebuild - INICIO", ['number' => $number, 'bot_id' => $bot_id], ['module' => 'chat_api']);
    ...
    try {
      ...
      if (!$chat) {
        log::warning("chat/rebuild - Sin mensajes en BD", ['number' => $number, 'bot_id' => $bot_id], ['module' => 'chat_api']);
        response::error('No se pudo reconstruir el chat. Verifica que existan mensajes en la BD.', 404);
      }

      log::success("chat/rebuild - Reconstruido", ['number' => $number, 'bot_id' => $bot_id, 'messages' => count($chat['messages'] ?? [])], ['module' => 'chat_api']);
      ...
      log::error("chat/rebuild - Error: {$e->getMessage()}", ['number' => $number, 'bot_id' => $bot_id], ['module' => 'chat_api']);
      response::serverError('Error al reconstruir chat', IS_DEV ? $e->getMessage() : null);
    }
  ...
  $router->get('/show/{number}/{bot_id}', function($number, $bot_id) {
    ...
    if (!$chat) response::error('Chat no encontrado', 404);
  ...
  $router->delete('/delete/{number}/{bot_id}', function($number, $bot_id) {
    ...
    if (unlink($chatFile)) {
      log::info("chat/delete - Eliminado", ['number' => $number, 'bot_id' => $bot_id], ['module' => 'chat_api']);
      response::success(['deleted' => true, 'file' => basename($chatFile)], 'Chat JSON eliminado');
    } else {
      response::serverError('Error al eliminar chat JSON');
    }
  })->middleware($devMiddleware);
  ...
  $router->get('/validate/{number}/{bot_id}', function($number, $bot_id) {
    ...
    $maxDays = request::query('max_days', 2);

...
// Verificación de webhook de Facebook Pages
// Meta hace un GET con hub_mode=subscribe, hub_verify_token y hub_challenge igual que WhatsApp
$router->get('/api/webhook/facebook', function() {
  $mode      = $_GET['hub_mode'] ?? null;
  $token     = $_GET['hub_verify_token'] ?? null;
  $challenge = $_GET['hub_challenge'] ?? null;
...
// Webhook de Facebook Pages (comentarios, mensajes, etc.)
//$router->post('/api/webhook/facebook', 'webhook@facebook')->middleware(['json']);
  $router->post('/api/webhook/facebook', 'webhook@facebook');
